...
export default function AccountList({ accounts, loadError }: { accounts: AliyunAccount[]; loadError?: string }) {
  ...
  return <>
    ...
    <Modal title={editingAccount ? "编辑阿里云账号" : "新增阿里云账号"} open={open} confirmLoading={saving} okText={editingAccount ? "保存修改" : "保存账号"} cancelText="取消" onOk={() => form.submit()} onCancel={closeModal} destroyOnHidden afterOpenChange={(visible) => { if (!visible) return; form.resetFields(); form.setFieldsValue(editingAccount ? { name: editingAccount.name, site: editingAccount.site, accessKeyId: editingAccount.accessKeyId, accessKeySecret: "" } : { site: "china" }); }}>
