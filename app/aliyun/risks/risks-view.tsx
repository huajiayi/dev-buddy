...
export default function RisksView({ data }: { data: AccountRisks[] }) {
  ...
  return <>
    ...
    <div className="stats-grid">
      <Card><Statistic title="高风险" value={all.filter((item) => item.level === "critical").length} prefix={<ExclamationCircleOutlined />} valueStyle={{ color: "#cf1322" }} /></Card>
      <Card><Statistic title="需关注" value={all.filter((item) => item.level === "warning").length} prefix={<WarningOutlined />} valueStyle={{ color: "#d46b08" }} /></Card>
      <Card><Statistic title="优化建议" value={all.filter((item) => item.level === "info").length} prefix={<SafetyCertificateOutlined />} /></Card>
      <Card><Statistic title="健康账号" value={data.filter((item) => item.risks.length === 0 && item.unavailable.length === 0).length} prefix={<CheckCircleOutlined />} valueStyle={{ color: "#389e0d" }} /></Card>
    </div>
    {data.length === 0 ? <Card className="detail-card"><Empty description="请先添加阿里云账号" /></Card> : <Collapse className="risk-collapse" defaultActiveKey={data.map((item) => item.account.id)} items={data.map((group) => ({
      ...
      children: <Space direction="vertical" size={14} className="risk-account-content">
        {group.unavailable.map((message) => <Alert key={message} type="warning" showIcon message="部分检查不可用" description={message} />)}
        <Text type="secondary">已完成：{group.checked.length ? group.checked.join("、") : "暂无可用检查项"}</Text>
        {group.risks.length ? <Table<CloudRisk> rowKey="id" columns={columns} dataSource={group.risks} pagination={false} scroll={{ x: 760 }} /> : <Alert type="success" showIcon message="当前未发现需要处理的风险" />}
      </Space>,
    }))} />}
  </>;
}
