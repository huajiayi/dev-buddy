...
export async function deleteDatabase(id: string) {
  try { await removeManagedDatabase(id); revalidatePath("/databases"); return { ok: true }; }
...
export async function toggleDatabase(id: string, enabled: boolean) {
  try { await setManagedDatabaseEnabled(id, enabled); revalidatePath("/databases"); return { ok: true }; }
...
export async function testDatabaseConnection(id: string) {
  try { await testManagedDatabase(id); return { ok: true }; }
